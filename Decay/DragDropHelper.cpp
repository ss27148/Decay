...
bool CDropDescription::SetText(DROPIMAGETYPE nType, LPCWSTR lpszText, LPCWSTR lpszText1)
{
	...
	if (IsValidType(nType))
	{
		...
		if (lpszText)
		{
			m_pDescriptions[nType] = _wcsdup(lpszText);
			bRet = true;
		}
		if (lpszText1)
		{
			m_pDescriptions1[nType] = _wcsdup(lpszText1);
			bRet = true;
		}
	}
	return bRet;
...
bool CDropDescription::SetInsert(LPCWSTR lpszInsert)
{
	...
	if (lpszInsert)
	{
		m_lpszInsert = _wcsdup(lpszInsert);
	}
	...
	if (lpszWide)
	{
		...
		if (NULL != hGlobal)
		{
			...
			else
			{
				...
				VERIFY(nDescLen == _snprintf_s(
					lpszBuf, nDescLen, nDescLen,
					"Version:1.0\r\nStartHTML:%010d\r\nEndHTML:%010d\r\nStartFragment:%010d\r\nEndFragment:%010d\r\n",
					nDescLen, 
